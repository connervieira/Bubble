...
if (count($addressUpdated->transactions) !== 0) { // Check if any transactions exist.
    if ((int)$cashp->getBlockchain()->getConfirmationCount($addressUpdated->transactions[0]) >= $required_confirmations) {
        if ((float)$addressUpdated->balance >= (float)$productsArray[$store_id][$productToPurchase]["price"]) {
